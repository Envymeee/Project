...
wb = load_workbook('D:\DUMMYDATAS.xlsx', data_only=True) #Full directory better if less ang way
...
for row in range(2, 10): #(From row 2 to row 10 read)
    for col in range(3, 4): #(From column 3 only)
        char = get_column_letter(col)
...
for i in range(len(GradeFirstIllumination)):
    if GradeFirstIllumination[i] >= 300:
        if GradeFirstIllumination[i] <=500:
...
for row in range(2, 10): #(From row 2 to row 10 read)
    for col in range(9, 10): #(From column 3 only)
        char = get_column_letter(col)
...
GradeFirstWorkResultFirstPart= GradeFirstResultFirstPart
...
GradeFirstWorkResultSecondPart= GradeFirstResultSecondPart
print("GradeFirstWorkResultSecondPart")
print(GradeFirstWorkResultSecondPart)
...
(Good Classroom)
"""
GradeFirstEnrolled=ws['C14'].value #GET value from excel
...
print("AvailableClassroomGradeFirst")
print(AvailableClassroomGradeFirst) 
